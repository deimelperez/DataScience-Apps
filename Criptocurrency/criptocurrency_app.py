...
mp_dict = {'USD': 2, 'BTC': 0, 'ETH': 1}
currency_price_unit_indx = col1.selectbox(
    'Select currency for price', ('USD', 'BTC', 'ETH'))
currency_price_unit = mp_dict[currency_price_unit_indx]
...
def load_data():
    ...
    for i in listings:
        ...
        price.append(i['quotes'][currency_price_unit]['price'])
        percentChange1h.append(
            i['quotes'][currency_price_unit]['percentChange1h'])
        percentChange24h.append(
            i['quotes'][currency_price_unit]['percentChange24h'])
        i['quotes'][currency_price_unit]['percentChange24h']
        percentChange7d.append(
            i['quotes'][currency_price_unit]['percentChange7d'])
        marketCap.append(i['quotes'][currency_price_unit]['marketCap'])
        volume24h.append(i['quotes'][currency_price_unit]['volume24h'])

    df = pd.DataFrame(columns=['coin_name', 'coin_symbol', 'marketCap', 'percentChange1h',
...
selected_coin = col1.multiselect(
    'Cryptocurrency', sorted_coin, ['BTC', 'ETH', 'DOGE'])

# Filtering data
df_selected_coin = df[(df['coin_symbol'].isin(selected_coin))]

# Sidebar - Number of coins to display
num_coin = col1.slider('Display Top N Coins', 1, 100, 3)
df_coins = df_selected_coin[:num_coin]

# Sidebar - Percent change timeframe
percent_timeframe = col1.selectbox('Percent change time frame',
                                   ['7d', '24h', '1h'])
percent_dict = {"7d": 'percentChange7d',
                "24h": 'percentChange24h', "1h": 'percentChange1h'}
...
df_change = pd.concat([df_coins.coin_symbol, df_coins.percentChange1h,
                       df_coins.percentChange24h, df_coins.percentChange7d], axis=1)
